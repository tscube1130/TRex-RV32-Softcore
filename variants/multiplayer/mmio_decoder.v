...
module mmio_decoder (
   ...
   output reg  [1:0]  audio_ev,          // To audio driver: 01=jump, 10=crash
    ...
    // --- NEW PORTS ---
    output reg         net_tx_o,      // Drives pmod_ja1_tx
    input  wire        net_rx_i,      // Reads pmod_ja2_rx
    input  wire        sw_role_i      // Reads SW[15]
);
    ...
    localparam [31:0] MMIO_NET_TX  = 32'h0000_2018; // NEW: Network Transmit
    localparam [31:0] MMIO_NET_RX  = 32'h0000_201C; // NEW: Network Receive
   ...
   always @(posedge clk or negedge reset) begin
        if (!reset) begin
            seg_data0 <= 32'hAAAA_AAAA;
            seg_data1 <= 32'hAAAA_AAAA;
            net_tx_o  <= 1'b0; // NEW: Pull TX low on reset
        end else begin
            ...
            // NEW: Latch CPU writes to the TX pin
            if (is_mmio_write && dmem_write_valid && (dmem_waddr == MMIO_NET_TX))
                net_tx_o <= dmem_wdata[0]; 
        end
    end

    // 3. UPDATE THE READ MUX
    always @(*) begin
        case (dmem_raddr)
            MMIO_SW_R:   mmio_rdata_o = {30'b0, dbl_jump_sticky, jump_sticky};
            MMIO_LFSR_R: mmio_rdata_o = {16'b0, lfsr_out};
            
            // NEW: Pack the role switch (bit 1) and RX pin (bit 0) into one read
            MMIO_NET_RX: mmio_rdata_o = {30'b0, sw_role_i, net_rx_i};
